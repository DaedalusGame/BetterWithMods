...
public class BlockSaw extends BTWBlock implements IMechanicalBlock
{
	...
	{
		IBlockState state = super.onBlockPlaced(world, pos, facing, flX, flY, flZ, meta, placer);
			...
			for(int i = 0; i < 3; i++)
			{
				...
				if(block == BWRegistry.aesthetic && blockState.getValue(BlockAesthetic.blockType).getMeta() < 2)
				{
					...
					if(blockState.getValue(BlockAesthetic.blockType).getMeta() == 0 && unobstructed)
						world.setBlockState(pos2, BWRegistry.aesthetic.getActualState(blockState, world, pos2).withProperty(BlockAesthetic.blockType, BlockAesthetic.EnumType.CHOPBLOCKBLOOD));
					break;
				}
	...
	private void sawBlockInFront(World world, BlockPos pos, Random rand)
	{
		...
		if(SawInteraction.contains(block, harvestMeta)) {
			if(HardcoreWoodInteraction.contains(block, harvestMeta)) {
				ItemStack bark = HardcoreWoodInteraction.getBarkOverride(block, harvestMeta) != null ? HardcoreWoodInteraction.getBarkOverride(block, harvestMeta).copy() : new ItemStack(BWRegistry.bark, 1, 0);
				bark.stackSize += rand.nextInt(2);
				InvUtils.ejectStackWithOffset(world, pos2, bark);
				ItemStack sawdust = new ItemStack(BWRegistry.material, 2, 22);
				sawdust.stackSize += rand.nextInt(2);
				InvUtils.ejectStackWithOffset(world, pos2, sawdust);
			}
			if(SawInteraction.getProduct(block, harvestMeta) != null) {
				InvUtils.ejectStackWithOffset(world, pos2, SawInteraction.getProduct(block, harvestMeta).copy());
			}

...
public class EntityItemBuoy extends EntityItem {
	...
	/**
	 * Used to access {@link EntityItem.searchForOtherItemsNearby} as it's private.
	 */
    private void superSearchForOtherItemsNearby(){
    	try {
			ReflectionHelper.findMethod(EntityItem.class, this, new String[]{"func_85054_d", "searchForOtherItemsNearby"},new Class<?>[]{}).invoke(this, (Object[]) null);
    ...
    private int getPickupDelay() {
    	return ReflectionHelper.getPrivateValue(EntityItem.class, this, new String[]{"field_70532_c", "delayBeforeCanPickup"});
    }
	...
	private void updateBuoy() {
		...
		{
			double low = getEntityBoundingBox().minY + (getEntityBoundingBox().maxY - getEntityBoundingBox().minY) * (double)(i + 0) * 0.375D + offset;
			double high = getEntityBoundingBox().minY + (getEntityBoundingBox().maxY - getEntityBoundingBox().minY) * (double)(i + 1) * 0.375D + offset;
			AxisAlignedBB boundingBox = new AxisAlignedBB(getEntityBoundingBox().minX, low, getEntityBoundingBox().minZ, getEntityBoundingBox().maxX, high, getEntityBoundingBox().maxZ);
	...
	/**
	 * Check the non visible current between two water blocks for all blocks nearby entity.
	 * @return
	 */
        ...
        for (int x = minX; x < maxX; ++x) {
            for (int y = minY; y < maxY; ++y) {
                for (int z = minZ; z < maxZ; ++z) {
                    if (checkBlockDrifting(x, y, z)) return true;
                }
            }
        }

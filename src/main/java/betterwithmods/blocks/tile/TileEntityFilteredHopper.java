...
public class TileEntityFilteredHopper extends TileEntityVisibleInventory implements IMechSubtype, IFilteredHopper
{
	...
	public void update()
	{
		...
		if(worldObj.getBlockState(pos).getBlock() instanceof BlockMechMachines)
			isOn = ((BlockMechMachines)BWRegistry.singleMachines).isMechanicalOn(this.worldObj, pos);
		entityCollision();
		if(isOn)
		{
	...
	{
		ItemStack filter = getStackInSlot(18);
		if(filter != null && filter.stackSize > 0)
		{
			if(filter.getItem() instanceof ItemBlock)
			{
				...
				if(filter.getItem() == Item.getItemFromBlock(BWRegistry.grate))
					return 3;
				if(filter.getItem() == Item.getItemFromBlock(BWRegistry.slats))
					return 5;
	...
	private boolean canFilterProcessItem(ItemStack stack)
	{
		...
		if(this.filterType > 0)
		{
			...
			if(this.filterType < 6)
			{
				if(item != Item.getItemFromBlock(Blocks.SAND) && !(item instanceof ItemSeeds) && !InvUtils.listContains(stack, OreDictionary.getOres("listAllseeds")) && item != Items.GUNPOWDER && item != Items.SUGAR && item != Items.BLAZE_POWDER && !InvUtils.listContains(stack, OreDictionary.getOres("foodFlour")))
				{
					if(!InvUtils.listContains(stack, InvUtils.dustNames))
					{
						...
						if(!InvUtils.listContains(stack, OreDictionary.getOres("string")) && item != Items.PAPER && (item == BWRegistry.material && stack.getItemDamage() != 28) && item != Item.getItemFromBlock(Blocks.WOOL))
						{
							int meta = stack.getItemDamage();
							if(this.filterType == 5) {
								return item == Items.LEATHER || item == Items.MAP || item == Items.FILLED_MAP || (item == BWRegistry.material && (meta == 1 || meta == 4 || (meta > 5 && meta < 10) || (meta > 31 && meta < 35)));
							}
							...
							if(item != Item.getItemFromBlock(Blocks.RED_FLOWER) && item != Item.getItemFromBlock(Blocks.YELLOW_FLOWER) && (!(item instanceof ItemBlock) && (stack.getMaxStackSize() > 1 && item != Items.FISHING_ROD && item != Items.CARROT_ON_A_STICK)))
							{
								...
								if(item != Items.BONE && item != Items.ARROW && !InvUtils.listContains(stack, OreDictionary.getOres("stickWood")) && !InvUtils.listContains(stack, InvUtils.cropNames) && item != Items.REEDS && item != Items.BLAZE_ROD && (item == BWRegistry.material && (meta != 8 && meta != 9)))
									return false;
							}
						}
					}
			...
			{
				return stack.getItem() == BWRegistry.material && (stack.getItemDamage() == 15 || stack.getItemDamage() == 23);
			}
	...
	private void entityCollision()
	{
		...
		if(!isXPFull())
		{
			BlockPos pos = new BlockPos(getXPos(), getYPos(), getZPos());
			if(captureXP(this))
	...
	private boolean captureDroppedItems(IFilteredHopper hopper)
	{
		if(TileEntityHopper.getCaptureItems(hopper.getWorld(), hopper.getXPos(), hopper.getYPos(), hopper.getZPos()).size() > 0)
		{
			boolean flag = false;
			for(EntityItem item : TileEntityHopper.getCaptureItems(hopper.getWorld(), hopper.getXPos(), hopper.getYPos(), hopper.getZPos()))
			{
				ItemStack stack = item.getEntityItem();
				if(hopper.filterType() == 4 && stack.getItem() == Item.getItemFromBlock(Blocks.GRAVEL))
				{
					handleGravelCase(item);
				}
				else if(hopper.filterType() == 6 && stack.getItem() == BWRegistry.material && (stack.getItemDamage() == 15 || stack.getItemDamage() == 23))
				{
					handleSoulCase(item);
				}
				else if(this.canFilterProcessItem(stack))
					flag = TileEntityHopper.putDropInInventoryAllSlots(this, item) || flag;
			}
			if(flag)
			{
	...
	private boolean captureXP(IFilteredHopper hopper)
	{
		if(getCollidingXPOrbs(hopper.getWorld(), hopper.getXPos(), hopper.getYPos(), hopper.getZPos()).size() > 0 && hopper.filterType() == 6)
		{
			boolean flag = false;
			for(EntityXPOrb orb : getCollidingXPOrbs(hopper.getWorld(), hopper.getXPos(), hopper.getYPos(), hopper.getZPos()))
			{
	...
	private void handleGravelCase(EntityItem item)
	{
		...
		if(redStack != 0)
		{
			...
			if(!TileEntityHopper.putDropInInventoryAllSlots(this, red))
			{
		...
		if(separate != 0)
		{
			...
			if(!TileEntityHopper.putDropInInventoryAllSlots(this, reg))
			{
	...
	private void handleSoulCase(EntityItem item)
	{
		...
		EntityItem hellfire = new EntityItem(this.getWorld(), item.lastTickPosX, item.lastTickPosY, item.lastTickPosZ, new ItemStack(BWRegistry.material, item.getEntityItem().stackSize, 16));
	...
	private void attemptToEjectStackFromInv()
	{
		...
		{
			ItemStack invStack = getStackInSlot(stackIndex);
			...
			else
			{
				...
				if(block == null || (!block.isBlockSolid(this.worldObj, down, EnumFacing.UP) && (this.worldObj.getTileEntity(down) == null || !(this.worldObj.getTileEntity(down) instanceof IInventory))))
					ejectIntoWorld = true;
				else
				{
					...
					if(tile != null && tile instanceof ISidedInventory)
					{
						...
						for(int slot : slots)
						{
							if(inv.canInsertItem(slot, ejectStack, EnumFacing.UP))
							{
								...
								decrStackSize(stackIndex, ejectStackSize);
							}
						}
					}
					else if(tile != null && tile instanceof IInventory)
					{
						...
						if(InvUtils.addItemStackToInv(inv, ejectStack))
							decrStackSize(stackIndex, ejectStackSize);
					}
					else
			...
			if(ejectIntoWorld)
			{
				...
				if(carts != null && carts.size() > 0)
				{
					for(int i = 0; i < carts.size(); i++)
					{
						...
						{
							int itemsStored;
							if(InvUtils.addItemStackToInv((IInventory)cart, ejectStack))
							...
							if(itemsStored > 0)
							{
								decrStackSize(stackIndex, itemsStored);
	...
	{
		boolean isOn = ((BlockMechMachines)BWRegistry.singleMachines).isMechanicalOn(this.worldObj, this.pos);
		BlockPos down = pos.down();
		if(this.filterType == 6)
		{
			...
			else if(soulsRetained > 7)
			{
				...
				if(worldObj.getBlockState(pos).getBlock() == BWRegistry.singleMachines)
	...
	private static List<EntityXPOrb> getCollidingXPOrbs(World world, double x, double y, double z)
	{
		return world.getEntitiesWithinAABB(EntityXPOrb.class, new AxisAlignedBB(x - 0.5D, y - 0.5D, z - 0.5D, x + 0.5D, y + 1.0D, z + 0.5D));
	}
